...
const OTPModal: React.FC<OTPModalProps> = ({ phoneNumber, onVerified, onClose }) => {
  ...
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="otp-title">
    <div className="w-full max-w-md overflow-hidden rounded-3xl bg-white shadow-2xl shadow-slate-950/30 transition-all duration-300 animate-[pulse_0.35s_ease-out_1]">
      ...
      <div className="px-6 py-7 sm:px-8"><p className="text-center text-sm leading-6 text-slate-500">We sent a one-time password to<br /><strong className="font-semibold text-navy">{maskedPhone}</strong></p>
        ...
        <div className="mt-7 rounded-2xl bg-slate-50 p-4 text-center"><p className="text-sm text-slate-600">{resendTimer > 0 ? <>Resend OTP in <strong className="tabular-nums text-navy">{resendTimer}</strong> seconds</> : 'Didn’t receive the OTP?'}</p><button onClick={resend} disabled={resendTimer > 0} className={`mx-auto mt-3 inline-flex items-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold transition ${resendTimer > 0 ? 'cursor-not-allowed bg-slate-200 text-slate-400' : 'bg-blue-600 text-white shadow-lg shadow-blue-600/20 hover:-translate-y-0.5 hover:bg-blue-700'}`}><MessageSquare size={16} />{resendTimer > 0 ? 'SMS Resend' : <><RefreshCw size={15} /> SMS Resend</>}</button></div>
        ...
        <button onClick={onVerified} disabled={otp.length !== 6} className={`mt-6 flex w-full items-center justify-center gap-2 rounded-xl py-3.5 font-semibold transition ${otp.length === 6 ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/25 hover:-translate-y-0.5 hover:bg-blue-700' : 'cursor-not-allowed bg-slate-200 text-slate-400'}`}><Check size={18} />Verify OTP</button>
      </div>
