...
export function useReportPurchaseGuard(product: ReportProduct, enabled = true) {
  ...
  useEffect(() => {
    if (enabled) void refreshPlans().catch(() => undefined)
  }, [enabled, refreshPlans])
  ...
  const ensureSufficientBalance = useCallback(async (): Promise<boolean> => {
    ...
    try {
      const livePlans = plans ?? await refreshPlans()
  ...
  }, [checkingBalance, openInsufficientModal, plans, product, refreshBalance, refreshPlans, wallet])
  ...
  const handleInsufficientApiError = useCallback(async (error: unknown): Promise<boolean> => {
    ...
    let livePrice = price
    try {
      const [latestWallet, latestPlans] = await Promise.all([refreshBalance(), refreshPlans()])
    ...
    return true
  }, [openInsufficientModal, price, product, refreshBalance, refreshPlans, wallet])
