    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-navy/55" onClick={onClose}>
      <motion.aside initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="ml-auto h-full w-full max-w-3xl overflow-y-auto bg-white" onClick={(event) => event.stopPropagation()}>
        ...
        <section className="p-8">
          ...
          <div className="my-8 grid grid-cols-1 gap-5 text-center sm:grid-cols-3 sm:gap-0"><b className="text-3xl text-[#d6a32f]">187<br /><span className="text-sm text-navy">AV Management workshops</span></b><b className="text-3xl text-[#d6a32f]">75,000<br /><span className="text-sm text-navy">Claims settled</span></b><b className="text-3xl text-[#d6a32f]">95%<br /><span className="text-sm text-navy">Happy customers</span></b></div>
          <div className="rounded bg-[#fff8e6] p-4 sm:p-6"><h3 className="mb-6 font-black">To file claim, call your claim manager and we'll do the rest</h3><div className="grid grid-cols-1 gap-4 text-center sm:grid-cols-2 lg:grid-cols-4">{['Co-ordinate with insurer & garage', 'Arrange all your paperwork', 'Find the best garage near you', 'Ensure highest claim payout'].map((item) => <p key={item}><Wrench className="mx-auto mb-2 text-[#d6a32f]" />{item}</p>)}</div></div>
