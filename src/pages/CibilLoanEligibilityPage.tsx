...
export default function CibilLoanEligibilityPage({ product = 'personal-loan' }: { product?: 'personal-loan' | 'credit-card' | 'business-loan' }) {
  ...
  const productLabel = isCreditCard ? 'a credit card' : isBusinessLoan ? 'a business loan' : 'a loan'

  return (
    <main className="min-h-[55vh] bg-slate-50 px-4 py-10 md:py-16">
      <div className="container-pb">
        <button onClick={() => navigate(formRoute)} className="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-navy transition hover:bg-slate-100">
        ...
        <section className="mx-auto mt-8 max-w-2xl rounded-2xl border border-emerald-100 bg-white px-6 py-14 text-center shadow-sm md:px-12">
          ...
          <h1 className="mt-3 font-serif text-3xl font-bold text-navy md:text-4xl">You are not eligible to apply for {productLabel}</h1>
        </section>
      </div>
    </main>
  )
